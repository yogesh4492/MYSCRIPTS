...
class GDriveToS3Transfer:
    ...
    def authenticate_gdrive(self, credentials_file: str = 'credentials.json'):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                typer.echo("Refreshing credentials...")
                creds.refresh(Request())
            else:
                if not os.path.exists(credentials_file):
                    typer.echo(f"❌ Error: {credentials_file} not found!")
                    typer.echo("\nPlease follow these steps:")
                    typer.echo("1. Go to https://console.cloud.google.com/")
                    typer.echo("2. Create a project or select existing one")
                    typer.echo("3. Enable Google Drive API")
                    typer.echo("4. Create OAuth 2.0 credentials (Desktop app)")
                    typer.echo("5. Download credentials.json to this directory")
